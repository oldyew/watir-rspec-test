...
#Specific code to deal with the temp files created by IEDriverServer for internet explorer
Dir.glob(ENV["TEMP"].gsub("\\","/")+"/*IED*.tmp").each do |iedriver_profile|
  size += File.size(iedriver_profile)
  files += 1
  p (skip_delete ? "Found" : "Removing") + " TEMP file:" + iedriver_profile
  next if skip_delete
  FileUtils.rm_rf(iedriver_profile)
end
scale = "bytes"
scaled = size
if size > 2**30
...
p "Across " + directories.to_s + " webdriver-profile directories and " + files.to_s + " files"