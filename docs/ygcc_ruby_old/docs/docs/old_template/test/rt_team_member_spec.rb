require 'spec_helper'
require './lib/controller/wd/team member/team_member_list_actions'
require './lib/controller/wd/team member/edit member/edit_team_member_personal_details'
require './lib/model/team_member_model'
...
describe 'User' do
  it 'should create new Team Member', story: 'first' do |e|
    e.step 'open Team Member section and check it is opened' do
      ...
      team_member_page = element(css: 'h2.page-title')
      expect(team_member_page.text).to eq('Team Member Details')
    end
    e.step 'open Create Team Member and check it is opened' do
      open_create_team_member
      create_team_member_page = element(css: 'div.page-title')
      expect(create_team_member_page.text).to eq('Team Member Details: ')
    end
    e.step "provide Team Member personal info and check Member First Name is #{$member_fn}" do
      ...
      firstname = text_field(id: 'firstname')
      expect(firstname.value).to eq($member_fn)
    end
    e.step 'save Team Member details and check Success notification appears' do
      save_team_member_details
      info_message = element(css: 'div.ygcc-notification__message')
      expect(info_message.text).to eq('Details have been saved successfully')
    end
  ...
  xit 'should be able to see Team Member List', story: 'third' do |e|
